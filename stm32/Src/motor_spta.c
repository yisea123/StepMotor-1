...
float periodcnt=0;
#endif
MOTOR_CONTROL_SPTA StepMotor[4]={0};
...
void Initial_Motor(unsigned char MotorID, unsigned char StepDive,unsigned int maxposition , TIM_TypeDef* TIMx)
{
	...
	case 1:
		...
		pmotor_spta->DIR_PORT=MOTOR1_DIRPORT;
		pmotor_spta->DIR_pin =MOTOR1_DIRPIN;
	...
	case 2:
		...
		pmotor_spta->DIR_PORT=MOTOR2_DIRPORT;
		pmotor_spta->DIR_pin =MOTOR2_DIRPIN;
		pmotor_spta->EN_PORT =MOTOR2_ENPORT;
	...
	case 3:
		...
		pmotor_spta->DIR_PORT=MOTOR3_DIRPORT;
	...
	case 4:
		...
		pmotor_spta->DIR_PORT=MOTOR4_DIRPORT;
		pmotor_spta->DIR_pin =MOTOR4_DIRPIN;
		pmotor_spta->EN_PORT =MOTOR4_ENPORT;
		pmotor_spta->EN_pin  =MOTOR4_ENPIN;
	default:
		return;
		break;
	}
...
void Start_Motor_SPTA(unsigned char MotorID,unsigned char dir,unsigned int Degree)
{
	...
	if(dir==1)
	{
		GPIO_SetBits(pmotor_spta->DIR_PORT,pmotor_spta->DIR_pin);
	}else{
		GPIO_ResetBits(pmotor_spta->DIR_PORT,pmotor_spta->DIR_pin);
	}
...
void TIMX_IRQHandler_SPTA(MOTOR_CONTROL_SPTA *pmotor)
{
...
#endif
	/*拉低脉冲信号*/
	pmotor->PWM_PORT->BRR = pmotor->PWM_pin; 
	...
	{ 
		pmotor->step_count+=1;
		/*拉高脉冲信号产生一个步进脉冲*/
		pmotor->PWM_PORT->BSRR = pmotor->PWM_pin; 
